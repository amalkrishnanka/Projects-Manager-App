...
import { projects, activityData, projectProgressData, tasks, teamMembers } from "../data/mockData";
...
const Dashboard = () => {
    ...
    return (
        <div className='w-screen h-screen bg-zinc-950 p-4 flex gap-4'>
            <Sidebar />
            <div className='w-full flex flex-col gap-4'>
                <Navbar />
                <main className='w-full h-full flex flex-col rounded-lg gap-4 overflow-y-auto overflow-x-hidden'>
                    {/* Overview Section */}
                    <section className='w-full flex gap-4 flex-wrap justify-center items-center'>
                        {overview.map((card) => (
                            <div className='bg-zinc-900 p-6 min-w-fit w-[25%] rounded-lg text-white flex flex-col flex-1 justify-center items-start gap-1'>
                                ...
                                {recentTasks.map((task) => (
                                    <div key={task.id} className='border-t border-t-zinc-600 flex flex-col justify-center py-4'>
                                        <span className='flex justify-between'>
                                            <h5>{task.title}</h5>
                                            <p className={`text-xs p-1 px-2 rounded-lg ${task.priority === 'high' ? ' bg-red-600' : (task.priority === 'medium' ? 'bg-orange-500' : 'bg-green-500')}`}>{task.priority.toUpperCase()}</p>
                                        </span>
